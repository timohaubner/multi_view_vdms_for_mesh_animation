...
def write_opencv_matrix(file, key, matrix):
    ...
    if matrix.ndim != 2:
        raise ValueError(
            f"{key} must be two-dimensional, got {matrix.shape}"
        )
...
def export_camera_parameters(
    ...
    if len(azimuths) != 5:
        raise ValueError(
            f"Exactly five angles are required, got {len(azimuths)}."
        )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export intri.yml and extri.yml for five cameras."
    )
    parser.add_argument(
        "--mesh_path",
        required=True,
    ...
    parser.add_argument(
        ...
        help=(
            "Five camera angles in degrees, "
            "assigned to 00, 01, 02, 03, and 04."
        ),
    )
    parser.add_argument(
        "--width",
