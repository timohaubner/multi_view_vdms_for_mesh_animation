...
def load_reference_views(
    ...
    for i, img_path in enumerate(img_paths, start=1):
        ...
        if crop_info is not None:
            img = apply_video_crop_to_reference_image(
                img,
                crop_info=crop_info,
                H=H,
                W=W,
        ...
        # DEBUG OUTPUT: save exactly the image that will be used as cond_view
        if debug_output_folder is not None:
            debug_dir = Path(debug_output_folder) / f"{base_count:06d}_cropped_reference_views"
            debug_dir.mkdir(parents=True, exist_ok=True)

            debug_path = debug_dir / f"reference_view_{i}_cropped.png"
            img.convert("RGB").save(debug_path)
            print(f"Saved cropped reference view {i}: {debug_path}")

        tensor = ToTensor()(img.convert("RGB")).unsqueeze(0).to(device)
...
def sample(
    ...
    processed_input_path, crop_info = preprocess_video(
        input_path,
        remove_bg=remove_bg,
        n_frames=n_frames,
        W=W,
