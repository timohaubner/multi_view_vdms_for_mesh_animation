...
class MeshExporter:
    def __init__(self, smpl_male_path: Path, smple_female_path: Path, export_path: Path):
        self.smpl_make_path = smpl_male_path
        self.smpl_female_path = smple_female_path
    ...
    def export_mesh(self, vertices, faces, output_path):
        for frame_index, frame_vertices in enumerate(vertices):
            frame_output_path = output_path / f"frame_{frame_index:0{3}d}.obj"

            self.export_obj(vertices=frame_vertices, faces=faces, output_path=frame_output_path)

            print(f"[{frame_index + 1}/{len(vertices)}] " 
                  f"OBJ saved: {frame_output_path}")


    def export_meshes(self, gt: GTSequence, result):
        if gt.gender == "male":
            smpl_model_path = self.smpl_make_path
        elif gt.gender == "female":
            smpl_model_path = self.smpl_female_path
        else:
            raise ("Unknown gender", gt.gender)
        ...
        gt_v = result["gt_v"]
        pred_v_coordinate_aligned = result["pred_v_initial_aligned"]
        pred_v_root_aligned = result["pred_v_centroid_aligned"]
        pred_v_pa_aligned = result["pred_v_pa_aligned"]

        self.export_mesh(vertices=gt_v, faces=faces, output_path=output_directory / "gt_v")
        self.export_mesh(vertices=pred_v_coordinate_aligned, faces=faces, output_path=output_directory / "pred_v_initial_aligned")
        self.export_mesh(vertices=pred_v_root_aligned, faces=faces, output_path=output_directory / "pred_v_centroid_aligned")
